...
def train():
    ...
    with procedure("Init Model") as p:
        model = Attention(input_dims=2048)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for each_iter, (imgs, labels, _) in enumerate(train_loader):
            input = imgs.squeeze().cuda()
            # print(input.size())
            target = labels.cuda()
            optimizer.zero_grad()
...
def val(model, val_loader, ceriterion, writer, epoch):
    model.eval()
    loss_val_meter = meter.AverageValueMeter()
    cm = meter.ConfusionMeter(2)

    with torch.no_grad():
        for each_iter, (imgs, labels, _) in enumerate(val_loader):
            input = imgs.squeeze().cuda()
            target = labels.cuda()
